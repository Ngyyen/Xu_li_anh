...
void do_the_program(int,int,int);

int main()
{
    vector<int> vec_index = { 0, 20, 40, 55, 70, 75, 80, 84, 90, 99 };
    vector<int> vec_k = { 5,10,20 };
    for (int i = 0; i<vec_index.size(); ++i)
        for (int j = 0; j < vec_k.size(); ++j) {
            do_the_program(vec_index[i], vec_k[j], 0);
            do_the_program(vec_index[i], vec_k[j], 1);
        }
    return 0;
}
...
void do_the_program(int index, int k, int metric)
{
    ...
    */
    ...
    string a = "_"+to_string(index), b ="_"+to_string(k), c ="_"+to_string(metric);
    imwrite("C:/Users/Nguyen Le/OneDrive/Máy tính/KQTV/Ketquatruyvan"+a+b+c+".jpg", mutiple_images);
